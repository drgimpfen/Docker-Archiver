"""DEPRECATED
This legacy routes file was replaced by `app/routes/api/archives.py`.
Please remove `app/routes/archives.py` from the repository.
"""
raise RuntimeError("Deprecated: 'app/routes/archives.py' is no longer used; delete the file and use '/api/archives' instead.")
...
from app.utils import setup_logging, get_logger, get_log_dir, format_bytes, format_duration, get_disk_usage, to_iso_z, now, local_now, filename_safe
...
# Legacy redirects removed — `/archives/*` legacy endpoints were deprecated and have been
# removed to simplify routing. Clients should use `/api/archives/*` endpoints instead.
# If you need temporary compatibility, reintroduce redirects here.
...
def dry_run(archive_id):
    ...
    try:
        ...
        os.makedirs(jobs_dir, exist_ok=True)
        safe_name = utils.filename_safe(archive['name'])
        log_name = f"{archive_id}_dryrun_{safe_name}.log"
        log_path = os.path.join(jobs_dir, log_name)
        try:
            subprocess.Popen(cmd + ['--log-path', log_path], start_new_session=True)
